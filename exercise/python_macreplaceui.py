...
pdb.set_trace()
qtCreatorFile = "E:\PYTHON\pythongui\MacReplace\macreplace.ui" # Enter file here.
...
self.enter_butten.clicked.connect(self.enter_click)
mac = self.mac_box.toPlainText()
NO = "操作失败 -_-b"
YES = "修改成功！！！ (^O^)"
self.Rusult_box.setText("123")
def enter_click(self):
    NO = "操作失败 -_-b"
    YES = "修改成功！！！ (^O^)"
    while True:
        mac = self.mac_box.toPlainText()
        self.Rusult_box.setText(YES)
        start = time.time()
        try:
        ...
        #print("\t%.2f秒" % (end-start))
        self.RUNTIME_box.setText(RUNTIME)
